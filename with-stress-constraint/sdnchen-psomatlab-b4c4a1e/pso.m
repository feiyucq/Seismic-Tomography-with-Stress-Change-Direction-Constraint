...
if strcmpi(options.UseParallel,'always')
    ...
    elseif isempty(ver('distcomp')) % Check for toolbox installed
        ...
        if isempty(gcp('nocreate'))
            parpool;
%             cqmatlabpoolsize=matlabpool('size')
        else
%             cqmatlabpoolsize=matlabpool('size')
        end
...
% !echo insert new to PSO
% state.Population=PSO_insertone(state.Population);
tic
for k = 1:itr
	...
	%insert some individual,para is state.Population,double, row is the number of individual , column is the number of unknows
	%cq changed
	!echo evaluate fitness start 
    tic;
    sprintf('PSO start!gen=%d',state.Generation)
	gen=state.Generation;
    if k==1%初代的时候插入一个初值，或者用404行代码插入初值
        %state.Population=PSO_insertone(state.Population);
    end
    %cq changed 
...
%             matlabpool('addattachedfiles',{[pwd '\testfcns\nonlinearconstrdemo.m']}) ;
    ...
    toc
    sprintf('PSO STOP!gen=%d',gen)
   
    !echo evaluate fitness stop
    % ---------------------------------------------------------------------
...
%     tempchk = alpha <= 1.6 ;
    ...
    %cq added
    trace(k)=state;
    sprintf('globle best is:%d',state.fGlobalBest(k))
